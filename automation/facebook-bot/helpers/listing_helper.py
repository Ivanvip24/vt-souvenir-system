...
def publish_listing(data, listing_type, scraper):
	...
	# Wait for location dropdown and click first option
	time.sleep(2)  # Wait for location suggestions to load
...
def add_fields_for_item(data, scraper):
	...
	# First, click to open the category dropdown
	category_xpaths = [
		'//span[text()="Categoría"]',
		'//span[text()="Category"]',
		'//label[contains(text(),"Categoría")]',
		'//span[contains(text(),"Categoría")]'
	]
	...
	# Select category - try multiple approaches
	category = data['Category']
	category_mapping = {
		'Home & Garden': ['Hogar y jardín', 'Hogar', 'Home & Garden', 'Home'],
		'Electronics': ['Electrónica', 'Electronics'],
		'Clothing': ['Ropa', 'Clothing'],
		'Sports & Outdoors': ['Deportes', 'Sports'],
		'Toys & Games': ['Juguetes', 'Toys'],
		'Health & Beauty': ['Salud', 'Health', 'Belleza', 'Beauty']
	}
	category_options = category_mapping.get(category, [category])
	...
	for cat in category_options:
		cat_xpaths = [
			f'//span[text()="{cat}"]',
			f'//div[@role="option"]//span[text()="{cat}"]',
			f'//li//span[text()="{cat}"]'
		]
		for xpath in cat_xpaths:
			if scraper.find_element_by_xpath(xpath, False, 1):
				scraper.element_click_by_xpath(xpath)
				category_found = True
				break
		if category_found:
	...
	if not category_found:
		for cat in category_options:
			contains_xpaths = [
				f'//span[contains(text(),"{cat}")]',
				f'//div[@role="option"]//span[contains(text(),"{cat}")]',
				f'//div[contains(@class,"option")]//span[contains(text(),"{cat}")]'
			]
			for xpath in contains_xpaths:
				if scraper.find_element_by_xpath(xpath, False, 1):
					scraper.element_click_by_xpath(xpath)
					category_found = True
					break
	...
	# Last resort: click any available option in the dropdown
	if not category_found:
		time.sleep(0.5)
		dropdown_selectors = [
	...
	# If still not found, try pressing Enter or Escape and continue
	if not category_found:
		print("WARNING: Could not select category, trying to continue...")
...
def add_listing_to_multiple_groups(data, scraper):
	# If the groups field is empty, do nothing
	if not data['Groups'] or not data['Groups'].strip():
		return
...
def post_listing_to_multiple_groups(data, listing_type, scraper):
	...
	# Post in different groups
	for group_name in group_names:
		# Remove whitespace and skip empty group names
		group_name = group_name.strip()
		if not group_name:
			continue
