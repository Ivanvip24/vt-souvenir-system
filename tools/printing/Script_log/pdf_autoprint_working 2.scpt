-- PDF Auto-Print with Working Scaling Options
-- Uses the successful SHARP-specific parameters for correct scaling
...
on printPDF(filePath, copyCount)
	try
		...
		set shellCommand to "lp -d \"" & printerName & "\" -n " & copyCount & " -o page-ranges=1 -o InputSlot=Bypass -o PageSize=Custom.330x480mm -o ARCOType=COTGraphics \"" & filePath & "\""

		do shell script shellCommand
		return true

	on error errMsg
		-- Fallback: ADHESIVO preset only if COTGraphics fails
		try
