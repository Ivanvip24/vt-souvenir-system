	...
	try
		...
		repeat with pdfFile in pdfFiles
			...
			if previewAndConfirm(filePath, pdfFile, processedCount + skippedCount + 1, fileCount) then
				-- Get copy count from user (Preview is now fully closed)
...
on previewAndConfirm(filePath, fileName, currentIndex, totalFiles)
	try
		...
		-- Close Preview completely AFTER getting user choice
		tell application "Preview"
			close every window
			quit
		end tell

		-- Make sure Preview is fully closed
		delay 1
...
on getCopyCount()
	try
		...
		set copyDialog to display dialog "How many copies do you need?" default answer "1" buttons {"Cancel", "Print"} default button "Print" with icon question giving up after 300
