(*
PDF Auto-Print & Archive System - Pre-Scaled Version WITH DEBUGGING
...
Original files remain unchanged; temporary scaled files are auto-deleted
*)

-- Configuration (as properties)
property watchFolder : "/Volumes/TRABAJOS/2025/ARMADOS VT/TO_PRINT"
property archivedFolder : "/Volumes/TRABAJOS/2025/ARMADOS VT/TO_PRINT/ARCHIVED"
property printPreset : "ADHESIVO"
property printerName : "SHARP_MX_6580N__8513615400_"
property scalingPercent : "100.2"
property scalingScript : "/Users/ivanvalenciaperez/Desktop/CLAUDE/BETA_PHASE/PRINTING_PJ/scale_pdf.py"
...
on run
	try
		...
		repeat with pdfFile in pdfFiles
			...
			set shouldPrint to previewAndConfirm(filePath, pdfFile, processedCount + skippedCount + 1, fileCount)

			display alert "DEBUG" message "Preview result: " & shouldPrint buttons {"OK"} default button "OK"

			if shouldPrint then
				-- Get copy count from user
				set copyCount to getCopyCount()

				display alert "DEBUG" message "Copy count: " & copyCount buttons {"OK"} default button "OK"

				if copyCount > 0 then
					-- Print the file with pre-scaling
					set printResult to printPDFScaled(filePath, copyCount, pdfFile)

					display alert "DEBUG" message "Print result: " & printResult buttons {"OK"} default button "OK"
...
on printPDFScaled(filePath, copyCount, fileName)
	set scaledPDF to ""
	try
		...
		display alert "DEBUG" message "Scaling command: " & scalingCommand buttons {"OK"} default button "OK"

		set scaledPDF to do shell script scalingCommand

		display alert "DEBUG" message "Scaled PDF created: " & scaledPDF buttons {"OK"} default button "OK"
		...
		-- Build lp command with ADHESIVO preset and copy count
		set printCommand to "lp -d " & quoted form of printerName
		set printCommand to printCommand & " -o \"preset=" & printPreset & "\""
		...
		display alert "DEBUG" message "Print command: " & printCommand buttons {"OK"} default button "OK"

		-- Execute print command
		try
			set printOutput to do shell script printCommand
			log "Print command output: " & printOutput

			display alert "DEBUG" message "Print output: " & printOutput buttons {"OK"} default button "OK"
		on error printErr
			display alert "DEBUG" message "Print command failed: " & printErr buttons {"OK"} default button "OK"

			-- If command-line printing fails, try opening in Preview
			log "Command-line print failed: " & printErr
			display notification "Using Preview fallback..." with title "PDF Auto-Print"

			tell application "Preview"
				open (scaledPDF as POSIX file)
				activate
			end tell

			delay 1

			-- Open print dialog for user
			...
			display dialog "Print dialog opened with scaled PDF." & return & return & "Please verify settings and click Print." & return & "The scaled temporary file will be deleted after printing." buttons {"Cancel", "Done Printing"} default button "Done Printing" with icon note

			if button returned of result is "Cancel" then
				tell application "Preview"
					close front window
				end tell
				error "User cancelled printing"
			end if
		...
		try
			...
			log "Deleted temporary scaled PDF: " & scaledPDF
		on error cleanupErr
	...
	on error errMsg
		...
		display alert "Print Function Error" message "Error in printPDFScaled: " & errMsg buttons {"OK"} default button "OK"
