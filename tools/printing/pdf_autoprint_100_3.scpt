-- PDF Auto-Print with 100.3% Scaling
	...
	try
		...
		repeat with pdfFile in pdfFiles
			...
			if previewAndConfirm(filePath, pdfFile) then
				...
				if copyCount > 0 then
					if printPDFWithDialog(filePath, copyCount) then
...
on printPDFWithDialog(filePath, copyCount)
	try
		-- Open PDF in Preview
		tell application "Preview"
			open (filePath as POSIX file)
		...
		set checklistMsg to "PRINT DIALOG VERIFICATION CHECKLIST:" & return & return
		set checklistMsg to checklistMsg & "✓ Printer: SHARP MX-6580N" & return
		set checklistMsg to checklistMsg & "✓ Preset: ADHESIVE" & return
		set checklistMsg to checklistMsg & "✓ Paper Size: ADHESIVE (330 x 480 mm)" & return
		set checklistMsg to checklistMsg & "✓ Pages: From 1 to 1" & return
		set checklistMsg to checklistMsg & "✓ Copies: " & copyCount & return & return
		set checklistMsg to checklistMsg & "PREVIEW SECTION (expand if needed):" & return
		set checklistMsg to checklistMsg & "✓ Auto Rotate: UNCHECKED" & return
		set checklistMsg to checklistMsg & "✓ Scale: " & targetScale & "%" & return
		set checklistMsg to checklistMsg & "✓ Print Entire Image: SELECTED" & return & return
		set checklistMsg to checklistMsg & "Click OK, then verify settings and click Print in the dialog."
