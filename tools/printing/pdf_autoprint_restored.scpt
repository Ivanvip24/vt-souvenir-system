-- PDF Auto-Print On-Demand Script
...
on printPDF(filePath, copyCount)
	try
		-- Simple print using lp command
		set shellCommand to "lp -d \"" & printPreset & "\" -n " & copyCount & " -o page-ranges=1 \"" & filePath & "\""
		do shell script shellCommand
		return true
	on error
		-- Fallback to Preview printing
		try
			tell application "Preview"
				open (filePath as POSIX file)
				activate
