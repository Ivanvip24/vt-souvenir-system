...
export async function processReengagementQueue() {
  try {
    ...
    log('info', 'followup.processing', { count: expired.rows.length });
    ...
    for (const conv of expired.rows) {
      try {
        ...
        if (lastMsg.rows.length > 0 && lastMsg.rows[0].direction === 'outbound') {
          ...
          log('info', 'followup.sent', { client: conv.client_name || conv.wa_id });
        } else {
          log('info', 'followup.skipped', { client: conv.client_name || conv.wa_id, reason: 'last message inbound' });
        }
        ...
        logError('followup.send.error', msgErr, { waId: conv.wa_id });
    ...
    logError('followup.scheduler.error', err);
  }
}
...
export function initializeFollowupScheduler() {
  ...
  log('info', 'followup.scheduler.ready', { schedule: '*/5 * * * *' });
}
...
export function stopFollowupScheduler() {
  if (scheduledJob) {
    scheduledJob.stop();
    scheduledJob = null;
    log('info', 'followup.scheduler.stopped');
  }
}
