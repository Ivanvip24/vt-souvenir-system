...
export function setModel(modelKey) {
  if (AVAILABLE_MODELS[modelKey]) {
    ...
    log('info', 'knowledge.model_changed', { model: AVAILABLE_MODELS[modelKey].name });
    return true;
  }
  return false;
}
...
export async function loadBrandContent() {
  log('info', 'knowledge.load.start', { path: AXKAN_PATH });
  ...
  for (const relativePath of MARKDOWN_FILES) {
    ...
    try {
      ...
      log('info', 'knowledge.load.file', { filename, chars: content.length });
    } catch (err) {
      log('warn', 'knowledge.load.file_error', { path: relativePath, error: err.message });
    }
  }
  ...
  log('info', 'knowledge.load.complete', { documents: documents.length, totalChars: fullContext.length });
...
export async function askQuestion(question, options = {}) {
  ...
  try {
    ...
    logError('knowledge.api_error', error);
    return {
      success: false,
      error: error.message
    };
