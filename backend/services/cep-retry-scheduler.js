...
export function initializeCepRetryScheduler() {
  ...
  if (retryEnabled === 'false') {
    log('info', 'cep.scheduler.disabled', { reason: 'CEP_RETRY_ENABLED=false' });
    return;
  }

  // Guard against double initialization
  if (scheduledJob) {
    log('warn', 'cep.scheduler.already_running');
    return;
  }

  log('info', 'cep.scheduler.init', { maxRetries: MAX_RETRIES, retryDelays: RETRY_DELAYS, schedule: '*/5 * * * *' });

  scheduledJob = cron.schedule('*/5 * * * *', async () => {
    try {
      await processPendingRetries();
    } catch (error) {
      logError('cep.scheduler.error', error);
    }
  }, {
    timezone: 'America/Mexico_City'
  });

  log('info', 'cep.scheduler.ready');
}
...
export function stopCepRetryScheduler() {
  if (scheduledJob) {
    scheduledJob.stop();
    scheduledJob = null;
    log('info', 'cep.scheduler.stopped');
  }
}
...
export async function processPendingRetries() {
  ...
  log('info', 'cep.retry.processing', { count: pending.length });
...
async function retryVerification(verification) {
  ...
  log('info', 'cep.retry.attempt', { attempt: newAttempt, maxRetries: MAX_RETRIES, orderNumber: order_number, claveRastreo: clave_rastreo });
  ...
  try {
    ...
    if (result && result.found) {
      ...
      log('info', 'cep.retry.found', { orderNumber: order_number, attempt: newAttempt });
      await onTransferFound(id, order_id, order_number, result);
    } else {
      // Transfer not found
      if (newAttempt >= MAX_RETRIES) {
        // Exhausted all retries
        log('warn', 'cep.retry.exhausted', { orderNumber: order_number, maxRetries: MAX_RETRIES });
    ...
    logError('cep.retry.error', error, { orderNumber: order_number });
...
async function onTransferFound(verificationId, orderId, orderNumber, result) {
  ...
  try {
    ...
    if (pdfBuffer && isGoogleDriveConfigured()) {
      ...
      if (uploadResult && uploadResult.success) {
        ...
        log('info', 'cep.pdf.uploaded', { orderNumber, pdfUrl });
      }
    }
  } catch (pdfError) {
    logError('cep.pdf.error', pdfError, { orderNumber });
  ...
  if (orderResult.rows.length > 0) {
    ...
    if (order.approval_status === 'pending_review') {
      ...
      log('info', 'cep.order.auto_approved', { orderNumber });
    } else {
      ...
      log('info', 'cep.order.confirmed', { orderNumber, note: 'already approved' });
    }
  }
}
...
async function scheduleNextRetry(verificationId, currentRetryCount) {
  ...
  log('info', 'cep.retry.scheduled', { verificationId, retryCount: newRetryCount, delayMinutes });
}
