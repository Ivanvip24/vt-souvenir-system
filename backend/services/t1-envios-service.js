...
const T1_TRACKING_BASE = 'https://apiv2.t1envios.com/rastreo/public';
const T1_PRODUCTS_BASE = 'https://rvka77opcl.execute-api.us-east-1.amazonaws.com/Prod/products';
const T1_STORE_ID = '208742';
...
export async function getTrackingStatus(trackingNumber) {
  ...
  const resp = await fetch(`${T1_TRACKING_BASE}/estado-guia/${encodeURIComponent(trackingNumber)}`);
...
export async function getTrackingDetail(trackingNumber) {
  ...
  const resp = await fetch(`${T1_TRACKING_BASE}/detail-guia/${encodeURIComponent(trackingNumber)}`);
...
export async function searchProducts(name, page = 1, limit = 20) {
  ...
  const resp = await fetch(`${T1_PRODUCTS_BASE}/?${params}`);
