...
export async function sendDailyFollowUp() {
  try {
    log('info', 'designer.followup.start');
    ...
    if (result.rows.length === 0) {
      log('info', 'designer.followup.none');
      return;
    }
    ...
    for (const [designerId, designer] of Object.entries(designerTasks)) {
      if (!designer.phone) {
        log('warn', 'designer.followup.no_phone', { designerName: designer.name });
        continue;
      }
      ...
      try {
        ...
        if (templateResult.success) {
          log('info', 'designer.followup.sent', { designerName: designer.name, taskCount: designer.tasks.length, method: 'template' });
        } else {
          // Fallback to regular message (only works within 24h window)
          log('info', 'designer.followup.template_failed', { designerName: designer.name });
          ...
          log('info', 'designer.followup.sent', { designerName: designer.name, method: 'regular' });
        }
      } catch (err) {
        logError('designer.followup.error', err, { designerName: designer.name });
      }
    }

    log('info', 'designer.followup.complete');
  } catch (err) {
    logError('designer.followup.error', err);
  }
}
...
export async function triggerDailyReport() {
  try {
    log('info', 'designer.report.daily.start');
    ...
    if (!ivanPhone) {
      log('warn', 'designer.report.daily.no_phone', { reason: 'IVAN_WHATSAPP_NUMBER not set' });
    ...
    log('info', 'designer.report.daily.sent');

    return { success: true, pdfUrl, filename, delivered: true };
  } catch (err) {
    logError('designer.report.daily.error', err);
    throw err;
  }
}
...
export async function triggerWeeklyReport() {
  try {
    log('info', 'designer.report.weekly.start');
    ...
    const ivanPhone = process.env.IVAN_WHATSAPP_NUMBER;
    if (!ivanPhone) {
      log('warn', 'designer.report.weekly.no_phone', { reason: 'IVAN_WHATSAPP_NUMBER not set' });
      return { success: true, pdfUrl, filename, delivered: false };
    }

    await sendWhatsAppDocument(ivanPhone, pdfUrl, filename, 'Reporte semanal de produccion');
    log('info', 'designer.report.weekly.sent');

    return { success: true, pdfUrl, filename, delivered: true };
  } catch (err) {
    logError('designer.report.weekly.error', err);
    throw err;
  }
}
...
export async function triggerSalesDigest() {
  try {
    log('info', 'designer.sales_digest.start');
    ...
    const ivanPhone = process.env.IVAN_WHATSAPP_NUMBER;
    if (!ivanPhone) {
      log('warn', 'designer.sales_digest.no_phone', { reason: 'IVAN_WHATSAPP_NUMBER not set' });
    ...
    try {
      ...
      pdfDelivered = true;
      log('info', 'designer.sales_digest.pdf_sent');
    } catch (pdfErr) {
      logError('designer.sales_digest.pdf_error', pdfErr);
    }

    log('info', 'designer.sales_digest.sent');
    return { success: true, delivered: true, pdfDelivered, textMessage };
  } catch (err) {
    logError('designer.sales_digest.error', err);
    throw err;
  }
}
...
export function initializeDesignerScheduler() {
  log('info', 'designer.scheduler.init');

  // Sales coaching: removed from cron — run locally via Claude Code session
  // Script: node backend/scripts/sales-coaching-local.js
  ...
  log('info', 'designer.scheduler.job', { job: 'sales_digest', schedule: '0 9 * * *' });
  ...
  log('info', 'designer.scheduler.job', { job: 'daily_followup', schedule: '0 18 * * *' });
  ...
  log('info', 'designer.scheduler.job', { job: 'daily_report', schedule: '0 19 * * *' });
  ...
  log('info', 'designer.scheduler.job', { job: 'weekly_report', schedule: '0 9 * * 0' });

  // Nightly sales analysis: removed from cron — runs via Claude Code LaunchAgent
  // Script: node backend/scripts/nightly-learning-local.js

  log('info', 'designer.scheduler.ready', { jobCount: 4 });
}
...
export function stopDesignerScheduler() {
  ...
  scheduledJobs = [];
  log('info', 'designer.scheduler.stopped');
}
