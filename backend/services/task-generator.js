...
export async function generateTasksForOrder(orderId, newStatus) {
  try {
    ...
    if (templates.rows.length === 0) {
      log('info', 'task_gen.no_templates', { status: newStatus });
      return [];
    }
    ...
    if (orderResult.rows.length === 0) {
      log('error', 'task_gen.order_not_found', { orderId });
      return [];
    }
    ...
    for (const template of templates.rows) {
      ...
      if (existing.rows.length > 0) {
        log('info', 'task_gen.already_exists', { task: template.name, orderId });
        continue;
      }
      ...
      log('info', 'task_gen.created', { task: template.name, orderId });
    }

    return createdTasks;

  } catch (error) {
    logError('task_gen.error', error);
    throw error;
  }
}
...
export async function completePreviousDepartmentTasks(orderId, oldStatus) {
  try {
    ...
    if (result.rows.length > 0) {
      log('info', 'task_gen.auto_completed', { count: result.rows.length, orderId, department: previousDepartment });
    }

    return result.rows;

  } catch (error) {
    logError('task_gen.complete_prev_error', error);
    throw error;
  }
}
...
export async function onOrderStatusChange(orderId, oldStatus, newStatus) {
  log('info', 'task_gen.status_changed', { orderId, oldStatus, newStatus });
  ...
  try {
    ...
    logError('task_gen.status_change_error', error);
    throw error;
  }
}
...
export async function createManualTask({
  ...
  try {
    ...
    logError('task_gen.manual_create_error', error);
    throw error;
  }
}
...
export async function getDepartmentQueue(department) {
  try {
    ...
    logError('task_gen.queue_error', error);
    throw error;
  }
}
...
export async function getEmployeeTasks(employeeId, includeCompleted = false, department = null) {
  try {
    ...
    logError('task_gen.employee_tasks_error', error);
    throw error;
  }
}
...
export async function updateTaskStatus(taskId, status, employeeId = null) {
  try {
    ...
    logError('task_gen.update_error', error);
    throw error;
  }
}
...
export async function assignTask(taskId, assignedTo, assignedBy) {
  try {
    ...
    logError('task_gen.assign_error', error);
    throw error;
  }
}
