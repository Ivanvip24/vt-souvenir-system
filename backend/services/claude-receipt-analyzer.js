...
async function downloadImageAsBase64(imageUrl) {
  try {
    ...
    logError('claude_receipt.download_error', error);
    throw error;
  }
}
...
export async function analyzeReceipt(imageUrl, existingMaterials = []) {
  try {
    log('info', 'claude_receipt.analyze.start', { imageUrl });
...
Respond ONLY with valid JSON in this exact format:
{
  ...
  "confidence": "high", "medium", or "low"
    ...
    try {
      ...
      logError('claude_receipt.parse_error', parseError);
      return {
        success: false,
    ...
    log('info', 'claude_receipt.analyze.success', { items: extractedData.items?.length || 0, total: extractedData.grand_total });
    ...
    logError('claude_receipt.analyze.error', error);
    return {
      success: false,
      error: error.message
    };
...
export async function analyzeReceiptFromBase64(base64Data, mediaType, existingMaterials = []) {
  try {
    log('info', 'claude_receipt.analyze_base64.start');
...
Respond ONLY with valid JSON in this exact format:
{
  ...
  "confidence": "high", "medium", or "low"
    ...
    log('info', 'claude_receipt.analyze_base64.success');
    ...
    logError('claude_receipt.analyze_base64.error', error);
    return {
      success: false,
      error: error.message
    };
