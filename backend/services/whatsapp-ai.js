...
export async function setGlobalAiEnabled(enabled) {
  ...
  log('info', 'whatsapp.ai.toggle', { enabled: globalAiEnabled });
  ...
  try {
    ...
    logError('whatsapp.ai.persist_error', e);
  }
...
(async function loadGlobalAiState() {
  try {
    ...
    if (result.rows.length > 0) {
      ...
      log('info', 'whatsapp.ai.state_loaded', { enabled: globalAiEnabled });
    }
  } catch (e) {
    logError('whatsapp.ai.load_error', e);
  }
})();
...
export function setAiModel(model) {
  ...
  currentModel = model;
  log('info', 'whatsapp.ai.model_changed', { model });
  return true;
}
...
function loadConfig(filename) {
  try {
    ...
    logError('whatsapp.ai.config_missing', err, { filename });
    return '';
  }
}
...
async function parseAIResponse(responseText) {
  ...
  if (orderMatch) {
    try {
      ...
      logError('whatsapp.ai.parse_error', err, { block: 'CREATE_ORDER' });
    }
  ...
  for (const match of listMatches) {
    try {
      ...
      logError('whatsapp.ai.parse_error', err, { block: 'SEND_LIST' });
    }
  }
  ...
  for (const match of buttonMatches) {
    try {
      ...
      logError('whatsapp.ai.parse_error', err, { block: 'SEND_BUTTONS' });
    }
  }
  ...
  for (const match of docMatches) {
    try {
      ...
      logError('whatsapp.ai.parse_error', err, { block: 'SEND_DOCUMENT' });
    }
  }
  ...
  if (quoteGenMatch) {
    try {
      ...
      log('info', 'whatsapp.ai.quote_parsed', { data: generateQuoteData });
    } catch (err) {
      logError('whatsapp.ai.parse_error', err, { block: 'GENERATE_QUOTE' });
    }
    ...
    log('warn', 'whatsapp.ai.quote_truncated');
    ...
    if (truncatedMatch) {
      ...
      try {
        ...
        intent = 'quote_generation';
        log('info', 'whatsapp.ai.quote_recovered', { data: generateQuoteData });
      } catch (e) {
        log('error', 'whatsapp.ai.quote_recovery_failed', { json: jsonStr });
      }
    }
  ...
  if (reactMatch) {
    try {
      ...
      logError('whatsapp.ai.parse_error', err, { block: 'REACT' });
    }
  ...
  if (shippingMatch) {
    try {
      ...
      if (zip && zip.length === 5) {
        ...
        log('info', 'whatsapp.ai.shipping_check', { zip, isExtended });
      }
    } catch (err) {
      logError('whatsapp.ai.shipping_check_error', err);
    }
  ...
  if (locationMatch) {
    try {
      ...
      logError('whatsapp.ai.parse_error', err, { block: 'REQUEST_LOCATION' });
    }
  ...
  if (carouselMatch) {
    try {
      ...
      logError('whatsapp.ai.parse_error', err, { block: 'SEND_CAROUSEL' });
    }
  ...
  if (flowMatch) {
    try {
      ...
      logError('whatsapp.ai.parse_error', err, { block: 'SEND_FLOW' });
    }
  ...
  try {
    ...
    try {
      ...
      if (activeAssignments.rows.length > 0) {
        ...
        log('info', 'whatsapp.ai.design_portal_routed', { waId, orderId: assignment.order_id });
        // Don't skip AI — let it respond normally even during active design assignments
      }
    } catch (designPortalErr) {
      logError('whatsapp.ai.design_portal_error', designPortalErr);
    ...
    if (orderJson) {
      try {
        ...
        try {
          ...
          log('info', 'whatsapp.ai.followup_set', { followUpDays, totalPieces, waId });
        } catch (fuErr) {
          logError('whatsapp.ai.followup_error', fuErr);
        }
      } catch (orderError) {
        logError('whatsapp.ai.order_failed', orderError);
      ...
      try {
        ...
        if (parsedItems.length > 0) {
          ...
          try {
            ...
            log('info', 'whatsapp.ai.quote_pdf_uploaded', { pdfUrl });
          } catch (uploadErr) {
            logError('whatsapp.ai.cloudinary_upload_failed', uploadErr);
          ...
          try {
            ...
            for await (const page of pdfDoc) {
              ...
              log('info', 'whatsapp.ai.quote_image_uploaded', { quoteImageUrl });
              break; // Only first page
            }
          } catch (imgErr) {
            logError('whatsapp.ai.quote_image_error', imgErr);
          }
          ...
          log('info', 'whatsapp.ai.quote_pdf_saved', { pdfUrl });

          actionTaken = actionTaken || 'quote_generated';
          log('info', 'whatsapp.ai.quote_generated', { quoteNumber: quoteResult.quoteNumber, itemCount: quoteResult.itemCount, total: quoteResult.total });
        } else {
          log('warn', 'whatsapp.ai.quote_no_items', { quoteText });
        }
      } catch (quoteError) {
        logError('whatsapp.ai.quote_error', quoteError);
      }
    }
    ...
    if (finalReply.length > 500) {
      log('warn', 'whatsapp.ai.reply_truncated', { length: finalReply.length });
    ...
    logError('whatsapp.ai.process_error', error);
...
export async function ensureAiToggleColumn() {
  ...
  try {
    ...
    if (err.message?.includes('already exists')) {
      ...
      logError('whatsapp.ai.migration_warn', err, { column: 'ai_enabled' });
    }
  }
}
...
async function ensureInsightsColumns() {
  ...
  try {
    ...
    if (err.message?.includes('already exists')) {
      ...
      logError('whatsapp.ai.migration_warn', err, { column: 'insights' });
    }
  }
}
...
export async function generateConversationInsights(conversationId) {
  ...
  try {
    ...
    logError('whatsapp.ai.insights_parse_error', parseErr);
    insightsJson = {
      insights: [{
        category: 'info',
        icon: '\u26a0\ufe0f',
