...
export async function buildIndex() {
  log('info', 'knowledge_index.build.start', { path: AXKAN_PATH });
  ...
  for (const relativePath of MARKDOWN_FILES) {
    ...
    try {
      ...
      log('info', 'knowledge_index.indexed_doc', { filename, sections: documents[documents.length-1].sections.length });
    } catch (err) {
      log('warn', 'knowledge_index.index_error', { path: relativePath, error: err.message });
    }
  }
  ...
  for (const imageDir of IMAGE_DIRECTORIES) {
    ...
    try {
      ...
      log('info', 'knowledge_index.indexed_images', { dir: imageDir, count: imageFiles.length });
    } catch (err) {
      log('warn', 'knowledge_index.images_error', { dir: imageDir, error: err.message });
    }
  }
  ...
  log('info', 'knowledge_index.build.complete', { documents: documents.length, images: images.length });
