  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      stream.on('finish', () => {
        ...
        console.log(`\u2705 Daily designer report generated: ${filename}`);
        resolve({ filepath, filename, url });
      });

      stream.on('error', reject);
    } catch (error) {
      console.error('\u274C Error generating daily designer report:', error);
      reject(error);
    }
  });
}
...
export async function generateWeeklyReport(data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      stream.on('finish', () => {
        const url = `${getBaseUrl()}/designer-reports/${filename}`;
        console.log(`\u2705 Weekly designer report generated: ${filename}`);
        resolve({ filepath, filename, url });
      });

      stream.on('error', reject);
    } catch (error) {
      console.error('\u274C Error generating weekly designer report:', error);
      reject(error);
    }
  });
}
