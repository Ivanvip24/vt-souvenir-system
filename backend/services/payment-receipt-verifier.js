...
function initializeClient() {
  ...
  log('info', 'receipt.verifier.init');
  return anthropicClient;
}
...
async function downloadImageAsBase64(imageUrl) {
  try {
    ...
    logError('receipt.image.download_error', error);
    throw error;
  }
}
...
export async function verifyPaymentReceipt(receiptImageUrl, expectedAmount, orderNumber) {
  try {
    log('info', 'receipt.verify.start', { orderNumber, expectedAmount, receiptImageUrl });
...
Responde SOLO con el JSON estructurado.`;
    ...
    log('info', 'receipt.verify.ai_response', { length: responseText.length });
    ...
    try {
      ...
      logError('receipt.verify.parse_error', parseError);
      return {
        success: false,
        verified: false,
    ...
    log('info', 'receipt.verify.complete', { recommendation: result.recommendation, orderNumber });
    return result;

  } catch (error) {
    logError('receipt.verify.error', error);
    return {
      success: false,
      verified: false,
      error: error.message,
