...
function initializeVisionClient() {
  ...
  try {
    ...
    log('info', 'ocr.init.credentials', { length: credentialsEnv.length });
...
async function downloadImage(imageUrl) {
  try {
    ...
    logError('ocr.download_error', error);
    throw error;
  }
}
...
export async function extractTextFromReceipt(imageUrl) {
  try {
    log('info', 'ocr.extract.start', { imageUrl });
    ...
    log('info', 'ocr.extract.success', { textLength: fullText.length });
    ...
    logError('ocr.extract.error', error);
    return {
      success: false,
      fullText: '',
      lines: [],
...
export async function processReceipt(imageUrl, expectedDeposit) {
  log('info', 'ocr.process.start', { imageUrl, expectedDeposit });
  ...
  log('info', 'ocr.process.complete', { extractedAmount: amountResult.amount, expectedDeposit, match: validation.isValid, difference: validation.difference });
