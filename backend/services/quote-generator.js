  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (fs.existsSync(ASSETS.logo)) {
        ...
        catch (err) { log('warn', 'quote.logo_missing', { error: err.message }); }
      }
      y += 80;
      ...
      stream.on('finish', () => {
        log('info', 'quote.generated', { filename });
        resolve({
          filepath,
          filename,
          quoteNumber,
      ...
      stream.on('error', (error) => {
        logError('quote.write_error', error);
        reject(error);
      });

    } catch (error) {
      logError('quote.generate_error', error);
      reject(error);
    }
  });
}
...
export async function deleteQuote(filepath) {
  try {
    if (fs.existsSync(filepath)) {
      ...
      log('info', 'quote.deleted', { filepath });
      return true;
    }
    return false;
  } catch (error) {
    logError('quote.delete_error', error);
    return false;
  }
}
