...
const WHATSAPP_API_BASE = process.env.WHATSAPP_API_BASE || 'https://graph.facebook.com/v22.0';
...
async function sendTokenAlert(statusCode, errorBody) {
  ...
  if (!adminEmail) {
    log('error', 'whatsapp.token.no_admin_email');
    return;
  }
  ...
  try {
    ...
    log('warn', 'whatsapp.token.alert.sent', { to: adminEmail });
  } catch (emailErr) {
    logError('whatsapp.token.alert.fail', emailErr);
  }
}
...
export async function metaApiFetch(path, options = {}) {
  ...
  if (response.status === 401 || response.status === 403) {
    ...
    log('error', 'whatsapp.token.expired', { status: response.status, url, body: errorBody.substring(0, 200) });

    if (!tokenDead) {
      tokenDead = true;
      log('error', 'whatsapp.token.dead', { message: 'Bot cannot send messages until token is refreshed' });
    }
    ...
    tokenDead = false;
    alertSentAt = null;
    log('info', 'whatsapp.token.recovered');
  }
...
export async function sendWhatsAppMessage(to, text) {
  try {
    ...
    if (data.error) {
      log('error', 'whatsapp.send.api_error', { error: data.error });
...
export async function sendWhatsAppVideo(to, videoUrl, caption) {
  try {
    ...
    const data = await response.json();
    if (data.error) {
      log('error', 'whatsapp.video.api_error', { error: data.error });
      return { success: false, error: data.error };
    }
    log('info', 'whatsapp.video.sent', { to, videoUrl });
    return { success: true, data };
  } catch (err) {
    logError('whatsapp.video.failed', err);
    return { success: false, error: err.message };
  }
}
...
export async function sendWhatsAppListMessage(to, header, body, footer, buttonText, sections) {
  try {
    ...
    const data = await response.json();
    if (data.error) {
      log('error', 'whatsapp.list.api_error', { error: data.error });
    ...
    logError('whatsapp.list.failed', err);
...
export async function sendWhatsAppButtonMessage(to, body, buttons, header, footer) {
  try {
    ...
    const data = await response.json();
    if (data.error) {
      log('error', 'whatsapp.button.api_error', { error: data.error });
      const fallbackText = `${body}\n\n${truncatedButtons.map(b => `• ${b.reply.title}`).join('\n')}`;
      return { success: false, error: data.error, fallbackText };
    }
    return { success: true, data };
  } catch (err) {
    logError('whatsapp.button.failed', err);
    return { success: false, error: err.message, fallbackText: body };
  }
}
...
export async function sendWhatsAppCTAMessage(to, body, displayText, url, header, footer) {
  try {
    ...
    const data = await response.json();
    if (data.error) {
      log('error', 'whatsapp.cta.api_error', { error: data.error });
      return { success: false, error: data.error, fallbackText: `${body}\n\n${url}` };
    }
    return { success: true, data };
  } catch (err) {
    logError('whatsapp.cta.failed', err);
    return { success: false, error: err.message, fallbackText: `${body}\n\n${url}` };
  }
}
...
export async function sendWhatsAppReaction(to, messageId, emoji) {
  try {
    ...
    const data = await response.json();
    if (data.error) {
      log('error', 'whatsapp.reaction.api_error', { error: data.error });
      return { success: false, error: data.error };
    }
    return { success: true, data };
  } catch (err) {
    logError('whatsapp.reaction.failed', err);
    return { success: false, error: err.message };
  }
}
...
export async function sendWhatsAppLocationRequest(to, body) {
  try {
    ...
    const data = await response.json();
    if (data.error) {
      log('error', 'whatsapp.location.api_error', { error: data.error });
      return { success: false, error: data.error, fallbackText: body };
    }
    return { success: true, data };
  } catch (err) {
    logError('whatsapp.location.failed', err);
    return { success: false, error: err.message, fallbackText: body };
  }
}
...
export async function sendWhatsAppCarousel(to, cards) {
  ...
  for (const card of cards.slice(0, 5)) {
    try {
      ...
      logError('whatsapp.carousel.card_error', err, { card: card.name });
    }
  }
...
export async function sendWhatsAppFlow(to, flowId, flowToken, body, ctaText = 'Comenzar') {
  try {
    ...
    const data = await response.json();
    if (data.error) {
      log('error', 'whatsapp.flow.api_error', { error: data.error });
      return { success: false, error: data.error, fallbackText: body };
    }
    return { success: true, data };
  } catch (err) {
    logError('whatsapp.flow.failed', err);
    return { success: false, error: err.message, fallbackText: body };
  }
}
...
export async function sendWhatsAppTemplate(to, templateName, languageCode = 'es_MX', bodyParameters = []) {
  try {
    ...
    const data = await response.json();
    if (data.error) {
      log('error', 'whatsapp.template.api_error', { error: data.error });
      return { success: false, error: data.error };
    }
    return { success: true, data };
  } catch (err) {
    logError('whatsapp.template.failed', err);
    return { success: false, error: err.message };
  }
}
