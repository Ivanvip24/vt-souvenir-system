...
export async function generateReceipt(orderData) {
  return new Promise((resolve, reject) => {
    try {
      ...
      log('info', 'pdf.generate.start', { orderNumber: orderData.orderNumber });
      log('info', 'pdf.generate.amounts', { totalPrice, depositAmount, remainingBalance });
      ...
      if (fs.existsSync(LOGO_PATH)) {
        try {
          ...
          log('warn', 'pdf.logo_missing', { error: err.message });
        }
      }
      ...
      stream.on('error', (error) => {
        logError('pdf.write_error', error);
        reject(error);
      });

    } catch (error) {
      logError('pdf.generate_error', error);
      reject(error);
    }
  });
}
...
export async function deleteReceipt(filepath) {
  try {
    ...
    logError('pdf.delete_error', error);
    return false;
  }
}
