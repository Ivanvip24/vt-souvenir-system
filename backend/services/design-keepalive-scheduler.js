  ...
  try {
    ...
    for (const row of atRisk.rows) {
      try {
        ...
        sentCount++;
        log('info', 'keepalive.sent', { client: row.client_name || row.client_phone, assignmentId: row.assignment_id });

        // Rate limit: 350ms between messages
        await new Promise(r => setTimeout(r, 350));
      } catch (msgErr) {
        logError('keepalive.send.error', msgErr, { phone: row.client_phone });
      }
    }

    if (sentCount > 0) {
      log('info', 'keepalive.complete', { sentCount });
    }
  } catch (err) {
    logError('keepalive.scheduler.error', err);
  }
}
...
export async function getDesignWindowStatus() {
  try {
    ...
    logError('keepalive.window_status.error', err);
    return [];
  }
}
...
export function initializeDesignKeepAlive() {
  ...
  scheduledJob = cron.schedule('*/30 * * * *', async () => {
    log('info', 'keepalive.check.start');
    await processDesignKeepAlive();
  });

  log('info', 'keepalive.scheduler.ready', { schedule: '*/30 * * * *' });
