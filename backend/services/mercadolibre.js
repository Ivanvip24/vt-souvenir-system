...
const ML_AUTH_URL = process.env.ML_AUTH_URL || 'https://auth.mercadolibre.com.mx';
...
export async function exchangeCodeForTokens(authCode) {
  ...
  log('info', 'ml.oauth.exchange');

  const response = await fetchWithTimeout(`${ML_API_URL}/oauth/token`, {
    method: 'POST',
  ...
  if (!response.ok) {
    ...
    log('error', 'ml.oauth.exchange_failed', { body: errorText });
    throw new Error(`Failed to exchange code for tokens: ${response.status}`);
  }

  const data = await response.json();
  log('info', 'ml.oauth.token_received');
...
export async function refreshAccessToken() {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    log('error', 'ml.oauth.refresh_failed', { body: errorText });
...
export async function disconnect() {
  await query(`UPDATE ml_oauth_tokens SET is_active = false WHERE is_active = true`);
  tokenCache = null;
  log('info', 'ml.oauth.disconnected');
}
...
export async function createListing(productData, siteId = 'MLM') {
  log('info', 'ml.listing.create', { siteId });
  ...
  log('info', 'ml.listing.payload', { familyName: listingPayload.family_name, categoryId: listingPayload.category_id, price: listingPayload.price });
  ...
  const response = await retry(async () => {
    ...
    if (!res.ok) {
      ...
      log('error', 'ml.listing.api_error', { error: errorData.message });
  ...
  log('info', 'ml.listing.created', { mlItemId: result.id });
...
export async function updateListing(mlItemId, updateData) {
  log('info', 'ml.listing.update', { mlItemId });
  ...
  log('info', 'ml.listing.updated', { mlItemId });
  return result;
}
...
export async function pauseListing(mlItemId) {
  log('info', 'ml.listing.pause', { mlItemId });
  ...
  log('info', 'ml.listing.paused', { mlItemId });
  return await response.json();
}
...
export async function activateListing(mlItemId) {
  log('info', 'ml.listing.activate', { mlItemId });
  ...
  log('info', 'ml.listing.activated', { mlItemId });
  return await response.json();
}
...
export async function closeListing(mlItemId) {
  log('info', 'ml.listing.close', { mlItemId });
  ...
  log('info', 'ml.listing.closed', { mlItemId });
  return await response.json();
}
...
export async function updateStock(mlItemId, quantity) {
  log('info', 'ml.stock.update', { mlItemId, quantity });
  ...
  log('info', 'ml.stock.updated', { mlItemId });
  return await response.json();
}
...
export async function bulkUpdateStock(items) {
  log('info', 'ml.stock.bulk_start', { count: items.length });
  ...
  log('info', 'ml.stock.bulk_done', { success: successCount, total: items.length });
...
export async function bulkPublish(products, siteIds, options = {}) {
  log('info', 'ml.bulk_publish.start', { products: products.length, sites: siteIds.length });
  ...
  for (const product of products) {
    for (const siteId of siteIds) {
      try {
        ...
        logError('ml.bulk_publish.item_failed', error, { product: product.name, siteId });
        results.push({
          productId: product.id,
          productName: product.name,
          siteId,
  ...
  const successCount = results.filter(r => r.success).length;
  log('info', 'ml.bulk_publish.done', { success: successCount, total: results.length });
...
async function logSyncHistory(listingId, syncType, requestData, responseData, status, errorMessage = null) {
  try {
    ...
    logError('ml.sync_history.error', error);
  }
}
