...
router.post('/webhook', (req, res) => {
  ...
  (async () => {
    try {
      ...
      // Sales coaching: removed API call — run locally via Claude Code session
...
router.post('/conversations/:id/reply', authMiddleware, async (req, res) => {
  try {
    ...
    // Send via Meta API (text, image, video) and capture real wamid
    let sendResult = null;
    if (videoUrl) {
      sendResult = await sendWhatsAppVideo(waId, videoUrl, message || '');
    } else if (imageUrl) {
      sendResult = await sendWhatsAppImage(waId, imageUrl, message || '');
    } else if (message) {
      sendResult = await sendWhatsAppMessage(waId, message);
    }

    // Extract real WhatsApp message ID from API response
    // sendWhatsAppMessage returns { success, data: {messages:[{id}]} }
    // sendWhatsAppImage/Video returns { messages:[{id}] } directly
    const realWamid = (sendResult && sendResult.data && sendResult.data.messages && sendResult.data.messages[0])
...
// Responds immediately with 202, processes AI in the background to avoid Render 30s timeout
// ---------------------------------------------------------------------------
router.post('/conversations/:id/recap', authMiddleware, async (req, res) => {
  try {
    ...
    // Respond immediately — process AI in background to avoid 30s proxy timeout
    res.json({ success: true, message: 'Recap iniciado, el mensaje se enviará en segundos...' });

    // Background: AI processing + send + store
    (async () => {
      try {
...
// ---------------------------------------------------------------------------
// 18b. DELETE /conversations/:convId/messages/:msgId — Delete individual message
//      Also revokes from WhatsApp (delete for everyone) if it's an outbound msg
// ---------------------------------------------------------------------------
router.delete('/conversations/:convId/messages/:msgId', authMiddleware, async (req, res) => {
  try {
    const { convId, msgId } = req.params;
    // Fetch message first to get wa_message_id for revocation
    const msgResult = await query(
      'SELECT wa_message_id, direction FROM whatsapp_messages WHERE id = $1 AND conversation_id = $2',
      [msgId, convId]
    );
    ...
    // Delete from local database (WhatsApp Business API does not support revoking sent messages)
