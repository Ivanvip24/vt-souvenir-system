...
(function () {
  ...
  function initCountryMap(key) {
    ...
    var cw = container.offsetWidth - (isMobile ? 8 : 16);
    var aspect = isMobile ? 0.68 : (config.aspect || 0.6);
    ...
    buildStateColorMap(key);
    drawCountryMap();
    renderStatesList(key);
  ...
  function drawCountryMap() {
    ...
    // On mobile the states list below handles state selection, so pins + labels
    // are skipped to keep the map clean and readable.
    var skipPins = window.innerWidth <= 600;
    if (skipPins) return;
  ...
  // Build color map shared by canvas pins + states list
  function buildStateColorMap(key) {
    var groups = countryStateGroups[key] || {};
    var brandColors = [COLORS.rosa, COLORS.turquesa, COLORS.naranja, COLORS.verde];
    stateColorMap = {};
    Object.keys(groups).forEach(function (name, i) {
      stateColorMap[name] = brandColors[i % brandColors.length];
    });
  }

  // Render clickable list of states alongside the country map
  function renderStatesList(key) {
    var listEl = document.getElementById('mexico-states-list');
    if (!listEl) return;
    var groups = countryStateGroups[key] || {};
    ...
    // Clear using DOM (no innerHTML with dynamic content)
    while (listEl.firstChild) listEl.removeChild(listEl.firstChild);

    var header = document.createElement('div');
    header.className = 'msl-header';
    var hTitle = document.createElement('span');
    hTitle.className = 'msl-title';
    hTitle.textContent = 'Elige un estado';
    var hCount = document.createElement('span');
    hCount.className = 'msl-count';
    hCount.textContent = names.length + ' estados';
    header.appendChild(hTitle);
    header.appendChild(hCount);
    listEl.appendChild(header);

    var itemsWrap = document.createElement('div');
    itemsWrap.className = 'msl-items';

    names.forEach(function (name) {
      var count = groups[name].length;
      var color = stateColorMap[name] || COLORS.rosa;

      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'msl-item';
      btn.setAttribute('data-state', name);
      btn.style.setProperty('--pin-color', color);

      var pin = document.createElement('span');
      pin.className = 'msl-pin';
      pin.setAttribute('aria-hidden', 'true');

      var nameEl = document.createElement('span');
      nameEl.className = 'msl-name';
      nameEl.textContent = name;

      var badge = document.createElement('span');
      badge.className = 'msl-badge';
      badge.textContent = String(count);

      var arrow = document.createElement('span');
      arrow.className = 'msl-arrow';
