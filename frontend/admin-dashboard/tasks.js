...
(function() {
  ...
  // Labels for display (private to this module)
  const TASK_DEPT_LABELS = {
    design: 'Diseno',
  ...
  function populateEmployeeDropdowns() {
    ...
    if (filterSelect) {
      ...
      taskEmployees.forEach(emp => {
        filterSelect.innerHTML += `<option value="${emp.id}">${escapeTaskHtml(emp.name)} (${TASK_DEPT_LABELS[emp.department] || emp.department})</option>`;
      });
    ...
    if (assignSelect) {
      ...
      taskEmployees.forEach(emp => {
        assignSelect.innerHTML += `<option value="${emp.id}">${escapeTaskHtml(emp.name)} (${TASK_DEPT_LABELS[emp.department] || emp.department})</option>`;
      });
      ...
      <tr style="border-bottom: 1px solid #e5e7eb; cursor: pointer;" onclick="openTaskDetailModal(${task.id})">
        <td style="padding: 16px;">
          <div style="display: flex; align-items: flex-start; gap: 12px;">
            <div style="width: 4px; height: 40px; border-radius: 2px; background: ${TASK_PRIORITY_COLORS[task.priority] || '#9ca3af'};"></div>
            <div>
              <div style="font-weight: 600; color: #1f2937; margin-bottom: 4px;">${escapeTaskHtml(task.title)}</div>
              ${task.order_number ? `<div style="font-size: 12px; color: #6b7280;">Pedido #${task.order_number}</div>` : ''}
            </div>
          </div>
        </td>
        <td style="padding: 16px;">
          <span class="dept-badge dept-${task.department}">${TASK_DEPT_LABELS[task.department] || task.department}</span>
        </td>
        <td style="padding: 16px; color: #4b5563;">
          ${task.assigned_to_name ? escapeTaskHtml(task.assigned_to_name) : '<span style="color: #9ca3af;">Sin asignar</span>'}
        </td>
        <td style="padding: 16px; text-align: center;">
          <span class="priority-badge priority-${task.priority}">${TASK_PRIORITY_LABELS[task.priority] || task.priority}</span>
        </td>
        <td style="padding: 16px; text-align: center;">
          <span class="status-badge status-${task.status}">${TASK_STATUS_LABELS[task.status] || task.status}</span>
        </td>
    ...
    body.innerHTML = `
      <div style="display: grid; gap: 20px;">
        ...
        <div>
          ...
          <p style="margin: 4px 0; color: #4b5563;">${escapeTaskHtml(task.description)}</p>
        </div>
        ` : ''}
  ...
  async function quickChangeStatus(taskId) {
    ...
    try {
      ...
      if (data.success) {
        showToast(`Estado cambiado a: ${TASK_STATUS_LABELS[nextStatus]}`, 'success');
        loadTasksData();
      } else {
