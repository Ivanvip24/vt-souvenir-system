...
window.switchPriceTab = function(tabName) {
  ...
  // Load cost-sheet tab (hand-edited products/ CSVs)
  if (tabName === 'cost-sheet') {
    loadCostSheet();
  }
};

// ==========================================================================
// HOJA DE COSTOS — reads products/ CSVs via /api/prices/cost-sheet
// All DOM built with createElement/textContent (no innerHTML with user data).
// ==========================================================================

function fmtMoneyCS(v) {
...
// Small DOM helper: el('div', {style:'...', class:'...'}, [children])
function csEl(tag, attrs, children) {
...
function renderCostSheetSummary(summaryEl, s, cfg) {
  clearNode(summaryEl);
  const card = (label, valueText, valueColor, subText) => {
    const c = csEl('div', { class: 'card', style: 'padding:16px;' });
    c.appendChild(csEl('div', { style: 'font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:0.05em;font-weight:600;margin-bottom:6px;', text: label }));
    c.appendChild(csEl('div', { style: `font-size:24px;font-weight:700;color:${valueColor};`, text: valueText }));
    if (subText) c.appendChild(csEl('div', { style: 'font-size:12px;color:#9ca3af;margin-top:4px;', text: subText }));
    return c;
  };
  const piecesPerDay = cfg && cfg.pieces_per_day ? cfg.pieces_per_day : '?';
  const workDays = cfg && cfg.work_days_per_month ? cfg.work_days_per_month : '?';
  summaryEl.appendChild(card('Costo por pieza', fmtMoneyCS(s.cost_per_piece_mxn), '#e72a88', `@ ${piecesPerDay} piezas/día × ${workDays} días`));
  summaryEl.appendChild(card('Productos', String(s.total_products || 0), '#111827', `${s.priced_products || 0} con precio · ${s.unpriced_products || 0} pendientes`));
  summaryEl.appendChild(card('Margen promedio (unitario)', fmtPctCS(s.avg_margin_unit_pct), marginColorCS(s.avg_margin_unit_pct)));
  summaryEl.appendChild(card('Margen promedio (mayoreo)', fmtPctCS(s.avg_margin_wholesale_pct), marginColorCS(s.avg_margin_wholesale_pct)));
}

function renderCostSheetProducts(productsEl, products) {
  clearNode(productsEl);
  if (!products || !products.length) {
    productsEl.appendChild(csEl('p', { style: 'color:#6b7280;text-align:center;padding:20px;', text: 'No hay productos en la hoja Products' }));
    return;
  }
  const table = csEl('table', { style: 'width:100%;border-collapse:collapse;font-size:13px;' });
  const thead = csEl('thead');
  const thL = 'text-align:left;padding:10px 8px;color:#6b7280;font-size:10px;text-transform:uppercase;letter-spacing:0.05em;white-space:nowrap;';
  const thR = 'text-align:right;padding:10px 8px;color:#6b7280;font-size:10px;text-transform:uppercase;letter-spacing:0.05em;white-space:nowrap;';
  thead.appendChild(csEl('tr', { style: 'border-bottom:2px solid #e5e7eb;' }, [
    csEl('th', { style: thL, text: 'SKU' }),
    csEl('th', { style: thL, text: 'Producto' }),
    csEl('th', { style: thR, text: 'Costo' }),
    csEl('th', { style: thR, text: 'Precio unit.' }),
    csEl('th', { style: thR, text: 'Gano / pza' }),
    csEl('th', { style: thR, text: 'Margen' }),
    csEl('th', { style: thR, text: 'Mayoreo' }),
    csEl('th', { style: thR, text: 'Gano / pza' }),
    csEl('th', { style: thR, text: 'Margen' })
  ]));
  table.appendChild(thead);
  const tbody = csEl('tbody');

  products.forEach(p => {
    const tr = csEl('tr', { style: 'border-bottom:1px solid #f3f4f6;' });

    tr.appendChild(csEl('td', { style: 'padding:12px 8px;color:#6b7280;font-size:12px;font-weight:600;text-transform:uppercase;', text: p.sku || '' }));
    tr.appendChild(csEl('td', { style: 'padding:12px 8px;font-weight:600;color:#111827;', text: p.name || p.sku || '' }));
    tr.appendChild(csEl('td', { style: 'padding:12px 8px;text-align:right;font-variant-numeric:tabular-nums;color:#6b7280;', text: fmtMoneyCS(p.cost_per_piece_mxn) }));

    // Unit price block
    tr.appendChild(csEl('td', { style: 'padding:12px 8px;text-align:right;font-variant-numeric:tabular-nums;', text: fmtMoneyCS(p.price_unit_mxn) }));
    tr.appendChild(csEl('td', {
      style: `padding:12px 8px;text-align:right;font-variant-numeric:tabular-nums;font-weight:600;color:${marginColorCS(p.margin_unit_pct)};`,
      text: fmtMoneyCS(p.margin_unit_mxn)
    }));
    tr.appendChild(csEl('td', {
      style: `padding:12px 8px;text-align:right;font-variant-numeric:tabular-nums;font-weight:600;color:${marginColorCS(p.margin_unit_pct)};`,
      text: fmtPctCS(p.margin_unit_pct)
    }));

    // Wholesale price block
    tr.appendChild(csEl('td', { style: 'padding:12px 8px;text-align:right;font-variant-numeric:tabular-nums;', text: fmtMoneyCS(p.price_wholesale_mxn) }));
    tr.appendChild(csEl('td', {
      style: `padding:12px 8px;text-align:right;font-variant-numeric:tabular-nums;font-weight:600;color:${marginColorCS(p.margin_wholesale_pct)};`,
      text: fmtMoneyCS(p.margin_wholesale_mxn)
    }));
    tr.appendChild(csEl('td', {
      style: `padding:12px 8px;text-align:right;font-variant-numeric:tabular-nums;font-weight:600;color:${marginColorCS(p.margin_wholesale_pct)};`,
      text: fmtPctCS(p.margin_wholesale_pct)
    }));
...
function renderCostSheetMaterials(materialsEl, materials, totalCost) {
  clearNode(materialsEl);
  if (!materials || !materials.length) {
    materialsEl.appendChild(csEl('p', { style: 'color:#6b7280;text-align:center;padding:20px;', text: 'No hay materiales en la hoja Materials' }));
    return;
  }
  const table = csEl('table', { style: 'width:100%;border-collapse:collapse;font-size:13px;' });
  const thL = 'text-align:left;padding:10px 8px;color:#6b7280;font-size:10px;text-transform:uppercase;letter-spacing:0.05em;white-space:nowrap;';
  ...
  const thead = csEl('thead');
  thead.appendChild(csEl('tr', { style: 'border-bottom:2px solid #e5e7eb;' }, [
    csEl('th', { style: thL, text: 'Material' }),
    csEl('th', { style: thR + 'color:#111827;', text: 'Costo / pieza' }),
    csEl('th', { style: thL, text: 'Variable' }),
    csEl('th', { style: thR, text: 'Costo bulk' }),
    csEl('th', { style: thL, text: 'Divisores' }),
    csEl('th', { style: thR, text: 'Mensual' }),
    csEl('th', { style: thL, text: 'Notas' })
  ]));
  table.appendChild(thead);
  const tbody = csEl('tbody');
  ...
  materials.forEach(m => {
    ...
    tr.appendChild(csEl('td', { style: 'padding:10px 8px;font-weight:600;color:#111827;', text: m.material || '' }));

    // cost_per_piece — BOLD, highlighted
    tr.appendChild(csEl('td', {
      style: 'padding:10px 8px;text-align:right;font-variant-numeric:tabular-nums;font-weight:700;color:#e72a88;font-size:14px;',
      text: fmtMoneyCS(m.cost_per_piece_mxn)
    }));

    // Variable flag
    const varCell = csEl('td', { style: 'padding:10px 8px;' });
    if (m.variable) {
      varCell.appendChild(csEl('span', {
        style: 'background:#fef3c7;color:#92400e;padding:3px 8px;border-radius:100px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;',
        text: 'variable'
      }));
    } else {
      varCell.appendChild(csEl('span', {
        style: 'background:#f3f4f6;color:#6b7280;padding:3px 8px;border-radius:100px;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;',
        text: 'fijo'
      }));
    }
    tr.appendChild(varCell);

    tr.appendChild(csEl('td', { style: 'padding:10px 8px;text-align:right;font-variant-numeric:tabular-nums;color:#6b7280;', text: m.bulk_cost_mxn != null ? fmtMoneyCS(m.bulk_cost_mxn) : '—' }));

    const divs = [m.div1, m.div2, m.div3, m.div4].filter(d => d != null && d > 0);
    tr.appendChild(csEl('td', { style: 'padding:10px 8px;color:#6b7280;font-family:monospace;font-size:12px;', text: divs.length ? divs.join(' × ') : '—' }));

    tr.appendChild(csEl('td', { style: 'padding:10px 8px;text-align:right;font-variant-numeric:tabular-nums;color:#6b7280;', text: m.monthly_cost_mxn != null ? fmtMoneyCS(m.monthly_cost_mxn) : '—' }));

    tr.appendChild(csEl('td', { style: 'padding:10px 8px;color:#9ca3af;font-size:11px;max-width:300px;', text: m.notes || '' }));

    tbody.appendChild(tr);
  });

  // Total row
  const totalTr = csEl('tr', { style: 'border-top:2px solid #111827;background:#fef2f7;' });
  totalTr.appendChild(csEl('td', { style: 'padding:14px 8px;font-weight:700;color:#111827;text-transform:uppercase;letter-spacing:0.03em;font-size:12px;', text: 'TOTAL costo por pieza' }));
  totalTr.appendChild(csEl('td', {
    style: 'padding:14px 8px;text-align:right;font-variant-numeric:tabular-nums;font-weight:800;color:#e72a88;font-size:16px;',
    text: fmtMoneyCS(totalCost)
  }));
  totalTr.appendChild(csEl('td', { colspan: '5' }));
  tbody.appendChild(totalTr);

  table.appendChild(tbody);
  materialsEl.appendChild(table);
}

window.loadCostSheet = async function() {
  const loadingEl = document.getElementById('cost-sheet-loading');
  const productsEl = document.getElementById('cost-sheet-products');
  const materialsEl = document.getElementById('cost-sheet-materials');
  const summaryEl = document.getElementById('cost-sheet-summary');

  if (loadingEl) loadingEl.classList.remove('hidden');
  if (productsEl) clearNode(productsEl);

  try {
    const response = await fetch(`${API_BASE}/prices/cost-sheet`, {
      headers: getAuthHeaders()
    });
    const result = await response.json();
    ...
    const data = result.data;
    if (summaryEl) renderCostSheetSummary(summaryEl, data.summary || {}, data.config);
    if (productsEl) renderCostSheetProducts(productsEl, data.products || []);
    if (materialsEl) renderCostSheetMaterials(materialsEl, data.materials || [], data.cost_per_piece_total || 0);
  } catch (error) {
    if (loadingEl) loadingEl.classList.add('hidden');
    console.error('Error loading cost sheet:', error);
    if (productsEl) {
      clearNode(productsEl);
      productsEl.appendChild(csEl('p', { style: 'color:#dc2626;text-align:center;padding:20px;', text: 'Error: ' + error.message }));
    }
  }
};
