 ...
 */
function openT1Envios(dataJson) {
  ...
  var url = 'https://shipping.t1.com/shippings/create-shipping#axkan=' + encoded;
  window.open(url, '_blank');

  if (!localStorage.getItem('t1_bookmarklet_installed')) {
    showNotification('T1 abierto. Usa el bookmarklet "AXKAN a T1" para llenar los campos.', 'info');
  }
}

// Generate bookmarklet code for T1 auto-fill
function getT1Bookmarklet() {
  return "javascript:void(function(){try{var h=location.hash;if(!h||!h.includes('axkan=')){alert('No hay datos de AXKAN. Abre T1 desde el dashboard primero.');return}var encoded=h.split('axkan=')[1];var json=decodeURIComponent(escape(atob(encoded)));var d=JSON.parse(json);function setVal(name,val){if(!val)return;var el=document.querySelector('input[name=\"'+name+'\"]');if(el){var nativeSet=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,'value').set;nativeSet.call(el,val);el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}))}}var names=d.name?d.name.split(' '):['',''];setVal('destination.fname',names[0]||'');setVal('destination.lname',names.slice(1).join(' ')||'');setVal('destination.email',d.email);setVal('destination.street',d.street);setVal('destination.extNumber',d.streetNumber);setVal('destination.postalCode',d.postal);var phoneInput=document.getElementById('phone-input');if(phoneInput&&d.phone){var ns=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,'value').set;ns.call(phoneInput,d.phone.replace(/\\D/g,'').slice(-10));phoneInput.dispatchEvent(new Event('input',{bubbles:true}));phoneInput.dispatchEvent(new Event('change',{bubbles:true}))}alert('Datos AXKAN llenados. Verifica CP y colonia.')}catch(e){alert('Error: '+e.message)}})()";
}

function showT1BookmarkletSetup() {
  var existing = document.getElementById('t1-bookmarklet-popup');
  if (existing) existing.remove();

  var popup = document.createElement('div');
  popup.id = 't1-bookmarklet-popup';
  popup.className = 'client-popup-overlay';
  popup.onclick = function(e) { if (e.target === popup) popup.remove(); };

  var content = document.createElement('div');
  content.className = 'client-popup-content';
  content.style.maxWidth = '500px';

  var header = document.createElement('div');
  header.className = 'client-popup-header';
  var title = document.createElement('h3');
  title.textContent = 'Configurar Bookmarklet T1';
  header.appendChild(title);
  var closeBtn = document.createElement('button');
  closeBtn.className = 'client-popup-close';
  ...
  body.className = 'client-popup-body';
  body.style.cssText = 'text-align:center;padding:20px;';

  var desc = document.createElement('p');
  desc.textContent = 'Arrastra este boton a tu barra de favoritos:';
  desc.style.marginBottom = '16px';
  body.appendChild(desc);

  var bookmarklet = document.createElement('a');
  bookmarklet.href = getT1Bookmarklet();
  bookmarklet.textContent = 'AXKAN a T1';
  bookmarklet.style.cssText = 'display:inline-block;background:#1a73e8;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px;cursor:grab;';
  bookmarklet.onclick = function(e) { e.preventDefault(); alert('Arrastrame a la barra de favoritos'); };
  body.appendChild(bookmarklet);

  var hint = document.createElement('p');
  hint.textContent = 'Despues, cuando abras T1 desde una direccion de cliente, haz click en el bookmarklet para llenar los campos automaticamente.';
  hint.style.cssText = 'margin-top:16px;color:#666;font-size:13px;';
  body.appendChild(hint);

  var doneBtn = document.createElement('button');
  doneBtn.textContent = 'Ya lo instale';
  doneBtn.style.cssText = 'margin-top:12px;padding:8px 20px;background:#e72a88;color:white;border:none;border-radius:6px;cursor:pointer;';
  doneBtn.onclick = function() { localStorage.setItem('t1_bookmarklet_installed', '1'); popup.remove(); showNotification('Listo', 'success'); };
          ...
          <div class="client-popup-section">
            ...
            ${hasFullAddress ? (() => {
              const t1Data = {
                ...
                phone: client.phone || '',
                email: client.email || '',
                street: (client.addresses && client.addresses[0]) ? (client.addresses[0].street || '') : (client.street || client.address || ''),
                streetNumber: (client.addresses && client.addresses[0]) ? (client.addresses[0].street_number || '') : (client.street_number || ''),
                colonia: (client.addresses && client.addresses[0]) ? (client.addresses[0].colonia || '') : (client.colonia || ''),
                city: (client.addresses && client.addresses[0]) ? (client.addresses[0].city || '') : (client.city || ''),
                state: (client.addresses && client.addresses[0]) ? (client.addresses[0].state || '') : (client.state || ''),
                postal: (client.addresses && client.addresses[0]) ? (client.addresses[0].postal || '') : (postal || ''),
                reference: (client.addresses && client.addresses[0]) ? (client.addresses[0].reference_notes || '') : (client.reference_notes || '')
              };
              const b64 = btoa(unescape(encodeURIComponent(JSON.stringify(t1Data))));
              return '<button onclick="openT1Envios(JSON.parse(decodeURIComponent(escape(atob(\'' + b64 + '\')))))" style="width:100%;margin-top:8px;padding:10px;background:#1a73e8;color:white;border:none;border-radius:8px;cursor:pointer;font-size:13px;font-weight:600;">🚚 Abrir en T1 Envios</button>';
            })() : ''}
          </div>
