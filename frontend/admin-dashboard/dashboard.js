...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Restore view from URL hash (e.g. #analytics, #employees-stats)
  ...
  if (hashView && document.getElementById(hashView + '-view')) {
    ...
    const navBtn = document.querySelector(`.nav-item[data-view="${hashView}"], .nav-sub-item[data-view="${hashView}"]`);
    if (navBtn) {
      navBtn.classList.add('active');
      const section = navBtn.closest('.nav-section');
...
function switchView(viewName) {
  // Update URL hash so refresh restores the current view
  ...
  if (viewName === 'whatsapp' && typeof loadWhatsAppConversations === 'function') {
    ...
    // Sales AI is a sub-view of WhatsApp — show WhatsApp view and immediately show Sales dashboard
    const waView = document.getElementById('whatsapp-view');
    if (waView) waView.classList.add('active');
    // Show Sales AI dashboard directly without loading WhatsApp conversations first
